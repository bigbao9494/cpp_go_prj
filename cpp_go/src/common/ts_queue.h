...
/*
使用自旋锁的线程安全队列 LFLock就是用atomic_flag来实现的自旋锁
*/
// 线程安全的队列(支持随机删除)
...
public:
    ...
    typedef typename std::conditional<ThreadSafe,std::lock_guard<LFLock>,fake_lock_guard>::type LockGuard;
    lock_t ownerLock_;
    lock_t *lock_;
    TSQueueHook* head_;
    TSQueueHook* tail_;
...
public:
	...
	ALWAYS_INLINE SList<T> pop_backWithoutLock(uint32_t n)
	{
		...
		//TSQueueHook* first = last;
		uint32_t c = 0;
		TSQueueHook* cur = nullptr;
		TSQueueHook* tail1 = tail_;
		TSQueueHook* tmpFirst = nullptr;
		TSQueueHook* tmpLast = nullptr;// &tmpFirst;
		//tmpFirst->next = tmpLast;
		//tmpLast->prev = &tmpFirst;
#if TSQueue_DEBUG
		first->check_ = nullptr;
#endif
		for (; c < n && tail1 != head_;)
		{
			...
			if (cur->moveable())
			{
				...
				{
					tmpFirst = cur;
					tmpFirst->prev = tmpFirst->next = nullptr;
				}
				tmpLast = cur;
		...
		count_ -= c;
		return SList<T>(tmpFirst, tmpLast, c);
	}
#endif
    ...
    {
		//避免已经被移除的task出现多次移除情况
		if (hook->prev == nullptr && hook->next == nullptr)
			return false;

		//检查hook对应的task本身所在的队列和移除它的当前队列是否为同一个，避免错误移除
	...
	//队列中添加一个元素并返回队列长度
    ALWAYS_INLINE size_t pushWithoutLock(T* element)
    {
