...
{
	int stack_size_;
    explicit __go_option(int v) : stack_size_(v) {}
	__go_option() : stack_size_(0) {} //为测试添加的默认构造函数
};
...
struct __go
{
	...
	//增加用户传递参数到协程函数中去
	template <typename Function>
	ALWAYS_INLINE void operator()(Function const& f, void* userParam)
	{
		if (!scheduler_) scheduler_ = Processer::GetCurrentScheduler();
		if (!scheduler_) scheduler_ = &Scheduler::getInstance();
		opt_.user_param_ = userParam;
		scheduler_->CreateTask(f, opt_);
	}
	template <typename Function>
	ALWAYS_INLINE void operator()(int stackSize,Function const& f, void* userParam)
	{
		if (!scheduler_) scheduler_ = Processer::GetCurrentScheduler();
		if (!scheduler_) scheduler_ = &Scheduler::getInstance();
		opt_.user_param_ = userParam;
		opt_.stack_size_ = stackSize;
		scheduler_->CreateTask(f, opt_);
	}
